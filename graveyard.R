...
target_location_string_vectorized <- function(cities, countries, empty = "") {
  return (if_else(cities == empty, 
          ...
          if_else(cities %exactlylike% " ", 
                  if_else(countries == empty, 
                          paste0("in ", cities), 
                          paste0("in ", cities, ", ", countries)), 
                  if_else(countries == empty, 
                          paste0('in <a href="https://en.wikipedia.org/wiki/', cities, '">', cities, "</a>"), 
                          paste0('in <a href="https://en.wikipedia.org/wiki/', cities, '">', cities, "</a>, ", countries)))))
}
...
unique_from_filter_slow2 <- function(war_tag, column, criteria) {
    arg_list <- c(list("war_tag" = war_tag), criteria)
    return (levels(unique(do.call(filter_selection2, arg_list)[[column]])))
  }

unique_from_filter2 <- memoise(unique_from_filter_slow2)

filter_selection2 <- function(war_tag, criteria) {
    datatable_part <- "war_data[[war_tag]]"
    multi_selectors <- criteria %>% keep(is.character) %>% (names(.)[. != "All"])
    if (is_empty(multi_selectors)) {
      selector_part <- ""
    } else {
      for (selector in multi_selectors) {
        eval(parse(text = paste0(selector, " <- criteria[[selector]]")))
      }
      selector_part <- paste0("[.(", paste0(multi_selectors, collapse = ", "), "), on = .(", unlist(dropdowns[multi_selectors], use.names = FALSE), ")]")
    }
    date_selectors <- criteria %>% keep(is.Date)
    if (is_empty(date_selectors)) {
      date_part <- ""
    } else {
      start_date <- date_selectors[["start_date"]]
      end_date <- date_selectors[["end_date"]]
      use_start_date <- !is.null(start_date) && start_date > first_missions[[war_tag]]
      use_end_date <- !is.null(end_date) && end_date < last_missions[[war_tag]]
      if (use_start_date) {
        if (use_end_date) {
          date_part <- "[Mission_Date >= start_date & Mission_Date <= end_date]"
        } else {
          date_part <- "[Mission_Date >= start_date]"
        }
      } else {
        if (use_end_date) {
          date_part <- "[Mission_Date <= end_date]"
        } else {
          date_part <- ""
        }
      }
    }
    eval(parse(text = paste0(datatable_part, selector_part, date_part)))
  }
...
filter_selection <- function(war_tag, start_date = earliest_date, end_date = latest_date, regions = "All", targets = "All", countries = "All", aircrafts = "All", weapons = "All") {
    if ("All" %c% countries) {
      if ("All" %c% aircrafts) {
        if ("All" %c% weapons) {
          war_data[[war_tag]][Mission_Date >= start_date & Mission_Date <= end_date]
        } else {
          war_data[[war_tag]][.(weapons), on = .(Weapon_Type)][Mission_Date >= start_date & Mission_Date <= end_date]
        }
      } else {
        if ("All" %c% weapons) {
          war_data[[war_tag]][.(aircrafts), on = .(Aircraft_Type)][Mission_Date >= start_date & Mission_Date <= end_date]
        } else {
          war_data[[war_tag]][.(aircrafts, weapons), on = .(Aircraft_Type, Weapon_Type)][Mission_Date >= start_date & Mission_Date <= end_date]
        }
      }
    } else {
      if ("All" %c% aircrafts) {
        if ("All" %c% weapons) {
          war_data[[war_tag]][.(countries), on = .(Unit_Country)][Mission_Date >= start_date & Mission_Date <= end_date]
        } else {
          war_data[[war_tag]][.(countries, weapons), on = .(Unit_Country, Weapon_Type)][Mission_Date >= start_date & Mission_Date <= end_date]
        }
      } else {
        if ("All" %c% weapons) {
          war_data[[war_tag]][.(countries, aircrafts), on = .(Unit_Country, Aircraft_Type)][Mission_Date >= start_date & Mission_Date <= end_date]
        } else {
          war_data[[war_tag]][.(countries, aircrafts, weapons), on = .(Unit_Country, Aircraft_Type, Weapon_Type)][Mission_Date >= start_date & Mission_Date <= end_date]
        }
      }
    }
  }
...
otherize_groups_prop <- function(dt, fact_name, group_name, cutoff, other = "other", both_ways = FALSE) {
  dt_rows <- nrow(dt)
  ...
  for (c in seq_len(nrow(for_removal))) {
    rows <- which(dt[[fact_name]] == for_removal[[fact_name]][[c]] & dt[[group_name]] == for_removal[[group_name]][[c]])
    set(dt, i = rows, j = fact_name, value = other)
    if (both_ways) {
      set(dt, i = rows, j = group_name, value = other)
    }
  }
}
...
sandbox_output <- function(war_tag) {
    ...
    renderPlot({
      ...
      plot_dep <- war_continuous[[war_tag]][[dep_input]]
      if (ind_input == all_data_label) {#***fix for categorical dep
        make_points_plot <- FALSE
        make_stats_plot <- TRUE
        make_mosaic_plot <- FALSE
        if (group_input == "None" || group_input %!c% grouping_features) {
        ...
        if (ind_input %c% continuous_features) {
          ...
          if (dep_input %c% continuous_features) {#***
            ...
            make_stats_plot <- TRUE
            make_mosaic_plot <- FALSE
            plot_ind <- categorical_colnames[[ind_input]]
            plot_dep <- continuous_colnames[[dep_input]]
          } else {
            make_points_plot <- FALSE
            make_stats_plot <- FALSE
            make_mosaic_plot <- TRUE
            plot_ind <- categorical_colnames[[ind_input]]
            plot_dep <- categorical_colnames[[dep_input]]
          }
        }
        if (group_input == "None" || group_input %!c% grouping_features) {
          ...
          grouped_plot <- TRUE
          plot_group <- grouping_colnames[[group_input]]
          graph_dt <- copy((war_selection[[war_tag]])()[, c(ind = plot_ind, dep = plot_dep, grp = plot_group), with = FALSE])
          setnames(graph_dt, c("ind", "dep", "grp"))
        ...
        if (make_stats_plot) {
          graph_dt[["ind"]] %>% 
            ...
            otherize_levels_count(cutoff = stat_count_cutoff, other = "others")
          if (grouped_plot) {
            graph_dt %>% otherize_groups_count("ind", "grp", cutoff = stat_count_cutoff, other = "others")
            if (calculate_plotted_items(graph_dt, "ind", "grp") > subset_graph_threshold) {
              graph_dt %>% otherize_groups_prop(cutoff = stat_prop_cutoff, 
                                                group_cols = c("ind", "grp"), 
                                                cols_to_otherize = "ind", 
                                                other = "others")
              if (calculate_plotted_items(graph_dt, "ind", "grp") > subset_graph_threshold) {
                graph_dt %>% otherize_groups_rank("ind", "grp", cutoff = stat_rank_cutoff, other = "others",  include_ties = FALSE)
              }
            }
          } else {
            if (calculate_plotted_items(graph_dt, "ind", "grp") > subset_graph_threshold) {
          ...
          graph_dt %>% otherize_groups_prop(cutoff = mosaic_prop_cutoff, 
                                            group_cols = cols, 
                                            cols_to_otherize = cols, 
                                            other = "others")
          graph_dt <- graph_dt[, .N, by = cols]
          sandbox_plot <- ggplot(data = graph_dt) + 
            geom_mosaic(mapping = aes(weight = N, x = product(dep, ind), fill = dep), offset = mosaic_offset) + 
            guides(fill = guide_legend(title = dep_input)) + 
            scale_y_continuous(labels = percent) + 
            coord_flip()
          if (grouped_plot) {
            sandbox_plot <- sandbox_plot + facet_grid(grp ~ .)
          }
        } else {
          sandbox_plot <- ggplot(data = graph_dt, mapping = aes(x = ind, y = dep))
        }
      ...
      if (make_points_plot) {
        ...
        if (transformation_vertical == "Logarithm") {
          ...
          # wanted to make trim = TRUE, but there's sometimes a fatal error when geom_violin draws quantiles on heavily skewed trimmed violins, 
          # I'm guessing because it trims before the quantile on the compressed side and doesn't know where to draw the quantile line
          sandbox_plot <- sandbox_plot + 
            geom_violin(draw_quantiles = c(0.25, 0.50, 0.75), trim = FALSE) + 
            stat_summary(fun.y = mean, geom = 'point', shape = 23, size = 2, position = position_dodge(width = 0.9))
        }
