...
test_that("no data column is completely empty or identical across all rows", {
  ...
  Korea_clean1 %>% select(-Weapon_Type, -Target_Country, -Target_Category) %>% map_int(~expect_gt(cardinality(.), 1L))
  Korea_clean2 %>% select(-Unit_Country, -Target_Country) %>% map_int(~expect_gt(cardinality(.), 1L))
